...
> Relocated from the repository README, which now documents the participant
> architecture. See [../README.md](../README.md) for how this submission builds
> the cofounder; the content below is the organizer's starter documentation and
> remains authoritative for setup, execution, and the freeze procedure.
...
- delete `AGENTS.md` and `MEMORY.md` at the repository root (context for coding
  agents working on the harness; Pi never reads them, because the runner passes
  `--no-context-files`);
- delete `tools/dev-skills/`, which holds editor-side skills that are never
  passed to Pi;
- keep `THIRD_PARTY_NOTICES.md` and every vendored `LICENSE.txt` — the skill
  under `solution/skills/vendor/` is Apache 2.0 and its licence must ship with it.

Run `npm run check` after the cleanup.
